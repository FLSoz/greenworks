...
public:
  ...
  public:
    ...
    virtual void OnPersonaStateChange(uint64 raw_steam_id, int persona_change_flag) = 0;
    virtual void OnAvatarImageLoaded(uint64 raw_steam_id, int image_handle, int height, int width) = 0;
    virtual void OnGameConnectedFriendChatMessage(uint64 raw_steam_id, int message_id) = 0;
    virtual void OnDLCInstalled(AppId_t dlc_app_id) = 0;
    virtual void OnMicroTxnAuthorizationResponse(uint32 AppID, uint64 OrderID, bool Autorized) = 0;
...
private:
  ...
  STEAM_CALLBACK(SteamClient, OnSteamServersConnected, SteamServersConnected_t, steam_servers_connected_);
  STEAM_CALLBACK(SteamClient, OnSteamServersDisconnected, SteamServersDisconnected_t, steam_servers_disconnected_);
  STEAM_CALLBACK(SteamClient, OnSteamServerConnectFailure, SteamServerConnectFailure_t, steam_server_connect_failure_);
  STEAM_CALLBACK(SteamClient, OnSteamShutdown, SteamShutdown_t, steam_shutdown_);
  STEAM_CALLBACK(SteamClient, OnPeronaStateChange, PersonaStateChange_t, steam_persona_state_change_);
  STEAM_CALLBACK(SteamClient, OnAvatarImageLoaded, AvatarImageLoaded_t, avatar_image_loaded_);
  STEAM_CALLBACK(SteamClient, OnGameConnectedFriendChatMessage, GameConnectedFriendChatMsg_t, game_connected_friend_chat_msg_);
